...
def create_submission(user_id: str, task_id: str, code: str, language: str = "python",
                      mcq_answer: int = None) -> dict:
    """
    ...
    SUPPORTED_LANGUAGES = {"python", "python3", "javascript", "java", "cpp", "c", "go", "rust", "typescript"}
    ...
    with transaction() as db:
        db.execute("""
            INSERT INTO submissions
                (id, user_id, task_id, code, language, status, submitted_at, mcq_answer)
            VALUES (?, ?, ?, ?, ?, 'pending', ?, ?)
        """, (submission_id, user_id, task_id, code, language, submitted_at, mcq_answer))
